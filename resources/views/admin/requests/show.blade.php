...
<style>
    ...
    .card-header { padding: .875rem 1.25rem; border-bottom: 1px solid var(--color-border); font-size: .9rem; font-weight: 700; color: var(--color-text); display: flex; align-items: center; justify-content: space-between; gap: .75rem; }
    .card-header-icon { color: #3b82f6; margin-right: .4rem; }
    ...
    .doc-status-icon { width: 32px; height: 32px; border-radius: 8px; display: flex; align-items: center; justify-content: center; font-size: .85rem; margin-bottom: .5rem; }
    ...
    .modal-alert { background: #fffbeb; border: 1px solid #fde68a; border-radius: 8px; padding: .75rem 1rem; font-size: .82rem; color: #854d0e; margin-bottom: 1rem; display: flex; gap: .5rem; }
...
@section('content')
    ...
    <div style="display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:.75rem; margin-bottom:1.25rem;">
        ...
        <div style="display:flex; gap:.5rem; flex-wrap:wrap;">
            <a href="{{ route('admin.requests.index') }}" class="btn btn-secondary btn-sm">← Retour</a>
    ...
    <div class="show-grid">
        ...
        <div>
            ...
            <div class="card">
                <div class="card-header">
                    <span><span class="card-header-icon">👤</span> Demandeur</span>
                    <a href="{{ route('admin.users.show', $fundingRequest->user) }}" class="btn btn-ghost-primary btn-sm">Voir le profil →</a>
                </div>
                <div class="card-body">
                    <div class="user-row">
            ...
            <div class="card">
                <div class="card-header">
                    <span><span class="card-header-icon">📄</span> Détails du projet</span>
                </div>
                <div class="card-body">
            ...
            <div class="card">
                <div class="card-header">
                    <span><span class="card-header-icon">📁</span> Documents ({{ count($documentsStatus) }})</span>
                    @php
                ...
                <div class="card-body">
                    ...
                    <div class="doc-cards-grid">
                        ...
                        <div class="doc-card {{ $dcClass }}">
                            <div class="doc-status-icon">{{ $dcIcon }}</div>
                            ...
                            @if($doc['provided'] ?? false)
                                ...
                                <div style="margin-top:.5rem;">
                                    <a href="{{ route('admin.documents.show', $doc['document_id'] ?? 0) }}" class="btn btn-sm btn-ghost-primary" style="font-size:.7rem; padding:.2rem .55rem;">Voir →</a>
                                </div>
                            @else
            ...
            {{-- ── Timeline ── --}}
            <div class="card">
                <div class="card-header"><span><span class="card-header-icon">🕐</span> Historique de la demande</span></div>
                <div class="card-body">
                    <div class="timeline">
                        ...
                        <div class="timeline-item">
                            ...
                            <div class="timeline-sub">Transaction : {{ $fundingRequest->kkiapay_transaction_id }}</div>
                        </div>
                        @endif
                        ...
                        <div class="timeline-item">
                            <div class="timeline-dot done"></div>
                            <div class="timeline-title">Financement débloqué</div>
                            <div class="timeline-date">{{ $fundingRequest->funded_at->format('d/m/Y à H:i') }}</div>
                        </div>
                        @endif

                        @if(in_array($fundingRequest->status, ['draft','pending','submitted','under_review','pending_committee']))
                        <div class="timeline-item">
            ...
            <div class="card">
                <div class="card-header"><span><span class="card-header-icon">⚙️</span> Actions disponibles</span></div>
                <div class="card-body">
                        ...
                        @switch($action)
                            @case('under_review')
                                <form method="POST" action="{{ route('admin.requests.status', $fundingRequest) }}">
                                    ...
                                    <button type="submit" class="action-btn a-review" style="width:100%;">
                                        <span class="action-icon"><svg fill="none" stroke="currentColor" viewBox="0 0 24 24" width="18" height="18"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg></span>
                            ...
                            @case('pending_committee')
                                <form method="POST" action="{{ route('admin.requests.status', $fundingRequest) }}">
                                    ...
                                    <button type="submit" class="action-btn a-commit" style="width:100%;">
                                        <span class="action-icon"><svg fill="none" stroke="currentColor" viewBox="0 0 24 24" width="18" height="18"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z"/></svg></span>
                                        <span class="action-label">Envoyer au comité</span>
                                    </button>
                                </form>
                                @break
                            @case('approved')
                                <button type="button" class="action-btn a-approve" onclick="document.getElementById('modal-approve').classList.add('open')" style="width:100%;">
                                    <span class="action-icon"><svg fill="none" stroke="currentColor" viewBox="0 0 24 24" width="18" height="18"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg></span>
                                    <span class="action-label">Approuver la demande</span>
                                </button>
                                @break
                            @case('rejected')
                                <button type="button" class="action-btn a-reject" onclick="document.getElementById('modal-reject').classList.add('open')" style="width:100%;">
                                    <span class="action-icon"><svg fill="none" stroke="currentColor" viewBox="0 0 24 24" width="18" height="18"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg></span>
                            ...
                            @case('funded')
                                <form method="POST" action="{{ route('admin.requests.status', $fundingRequest) }}">
                                    ...
                                    <button type="submit" class="action-btn a-fund" style="width:100%;" onclick="return confirm('Marquer comme financée ?')">
                            ...
                            @case('cancelled')
                                <form method="POST" action="{{ route('admin.requests.status', $fundingRequest) }}"
                                    ...
                                    <button type="submit" class="action-btn a-cancel" style="width:100%;">
                                        <span class="action-icon"><svg fill="none" stroke="currentColor" viewBox="0 0 24 24" width="18" height="18"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636"/></svg></span>
                    ...
                    <div style="margin-top:1.25rem; padding-top:1.25rem; border-top:1px solid var(--color-border);">
                        <div style="font-size:.82rem; font-weight:700; margin-bottom:.875rem; color:var(--color-text);">⚖️ Saisir la décision du comité</div>
        ...
        <div>
            ...
            <div class="amounts-card">
                ...
                @if($amounts['approved'] ?? false)
                <hr class="amounts-divider">
                <div class="amounts-label">Montant net à verser</div>
            ...
            {{-- Paiement --}}
            <div class="card">
                <div class="card-header"><span>💳 Paiement</span></div>
                <div class="card-body">
                    <div class="info-row">
            ...
            <div class="card">
                <div class="card-header"><span>🏢 Entreprise</span></div>
                <div class="card-body">
            ...
            @if($fundingRequest->reviewer)
            <div class="card">
                <div class="card-header"><span>👤 Assigné à</span></div>
                <div class="card-body">
            ...
            {{-- Notes internes --}}
            <div class="card">
                <div class="card-header"><span>📝 Notes internes</span></div>
                <div class="card-body">
    ...
    <div class="modal-backdrop" id="modal-reject">
        <div class="modal-box">
            <div class="modal-header-row">
                <div class="modal-title" style="color:#dc2626;">✕ Rejeter la demande</div>
                <button class="modal-close" onclick="document.getElementById('modal-reject').classList.remove('open')">
                    <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" width="18" height="18"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                </button>
            </div>
            <div class="modal-alert">
                ⚠️ Cette action est irréversible. Le demandeur sera notifié du rejet.
            </div>
            <form method="POST" action="{{ route('admin.requests.status', $fundingRequest) }}">
                @csrf @method('PATCH')
